...
export function NotificationBell() {
  ...
  const refresh = useCallback(async (): Promise<AppNotification[]> => {
    ...
    setError(null);
    try {
      const nextItems = await fetchNotifications(session.user.id);
      setItems(nextItems);
      return nextItems;
    } catch (err) {
      console.error('Failed to load notifications', err);
      setError(getUserErrorMessage(err, 'تعذر تحميل الإشعارات.'));
      return [];
    } finally {
      setLoading(false);
    }
