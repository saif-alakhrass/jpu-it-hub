...
const BATCH_COLUMNS = 'id, subject_id, tab, title, uploader_id, status, file_count, created_at';

export interface PaginatedFiles { items: FileRow[]; total: number; page: number; totalPages: number; }

export async function fetchFilesForSubject(subjectId: string, tab?: FileTab): Promise<FileRow[]> {
  let query = supabase.from('files').select(FILE_COLUMNS).eq('subject_id', subjectId);
...
export async function fetchBatchesForSubject(subjectId: string, tab?: FileTab): Promise<FileBatch[]> {
  let query = supabase.from('file_batches').select(BATCH_COLUMNS).eq('subject_id', subjectId);
  if (tab) query = query.eq('tab', tab);
  const { data, error } = await query.order('created_at', { ascending: false });
...
export async function fetchPendingFilesPaged(page: number): Promise<PaginatedFiles> {
  ...
  const { data, error, count } = await supabase.from('files').select(FILE_COLUMNS, { count: 'exact' }).eq('status', 'pending').order('created_at', { ascending: false }).range(from, to);
...
export async function fetchRejectedFilesPaged(page: number): Promise<PaginatedFiles> {
  const from = page * PAGE_SIZE;
  const to = from + PAGE_SIZE - 1;
  const { data, error, count } = await supabase.from('files').select(FILE_COLUMNS, { count: 'exact' }).eq('status', 'rejected').order('created_at', { ascending: false }).range(from, to);
...
export interface AdminStats { totalFiles: number; approvedFiles: number; pendingFiles: number; rejectedFiles: number; totalUsers: number; trustedUsers: number; admins: number; totalSubjects: number; totalBatches: number; }
...
export async function fetchAdminStats(): Promise<AdminStats> {
  ...
  const failed = [files, pending, rejected, users, trusted, admins, subjects, batches].find((result) => result.error);
  if (failed?.error) failService('fetch admin statistics', failed.error);
  return {
    totalFiles: files.count ?? 0, approvedFiles: (files.count ?? 0) - (pending.count ?? 0) - (rejected.count ?? 0),
    pendingFiles: pending.count ?? 0, rejectedFiles: rejected.count ?? 0, totalUsers: users.count ?? 0,
    trustedUsers: trusted.count ?? 0, admins: admins.count ?? 0, totalSubjects: subjects.count ?? 0, totalBatches: batches.count ?? 0,
  };
}
