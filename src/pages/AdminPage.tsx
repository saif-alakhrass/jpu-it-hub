...
export function AdminPage() {
  ...
  async function loadPending() {
    const { data } = await supabase
      .from('files')
      .select('id, subject_id, tab, title, storage_path, file_url, file_type, uploader_id, status, created_at, uploader:profiles!files_uploader_id_fkey(id, full_name, role), subject:subjects!files_subject_id_fkey(id, name)')
      .eq('status', 'pending')
  ...
  async function approve(id: string) {
    ...
    setPending((prev) => prev.filter((f) => f.id !== id)); setPreview(null);
  }
  ...
  if (authLoading) return (<div className="py-20 text-center"><Icon name="Loader2" className="mx-auto h-8 w-8 animate-spin text-brand-400" /></div>);
  if (!isAdmin) return (
    ...
    <div className="mx-auto max-w-5xl px-4 py-8">
      <header className="mb-6">
        <h1 className="flex items-center gap-2 text-2xl font-extrabold text-slate-100 md:text-3xl">
          <Icon name="ShieldCheck" className="h-7 w-7 text-accent-400" /> لوحة الإدارة
        </h1>
        <p className="mt-1 text-slate-400">مراجعة الملفات وإدارة الثقة والصلاحيات.</p>
      </header>
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-3">
        <div className="card p-4"><div className="text-xs text-slate-400">ملفات قيد المراجعة</div><div className="mt-1 text-2xl font-extrabold text-accent-400">{pending.length}</div></div>
        <div className="card p-4"><div className="text-xs text-slate-400">إجمالي المستخدمين</div><div className="mt-1 text-2xl font-extrabold text-slate-100">{students.length}</div></div>
        <div className="card p-4 col-span-2 md:col-span-1"><div className="text-xs text-slate-400">مستخدمون موثوقون</div><div className="mt-1 text-2xl font-extrabold text-brand-400">{students.filter((s) => s.role === 'trusted').length}</div></div>
      </div>
      <div className="mb-5 flex gap-2 border-b border-white/5">
        <button onClick={() => setTab('pending')} className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-bold transition ${tab === 'pending' ? 'border-accent-500 text-accent-400' : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
          <Icon name="Clock" className="h-4 w-4" /> قيد المراجعة ({pending.length})
        </button>
        <button onClick={() => setTab('users')} className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-bold transition ${tab === 'users' ? 'border-brand-500 text-brand-300' : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
          <Icon name="Users" className="h-4 w-4" /> المستخدمون والثقة
        </button>
      </div>
      {loading ? (
          ...
          <div className="grid gap-3">
            {pending.map((f) => (
              <div key={f.id} className="card flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => setPreview(f)} className="btn-ghost" title="معاينة"><Icon name="Eye" className="h-4 w-4" /></button>
                  <button onClick={() => approve(f.id)} className="btn-primary"><Icon name="Check" className="h-4 w-4" /> موافقة</button>
                  <button onClick={() => reject(f.id, f.storage_path)} className="btn-danger"><Icon name="Trash2" className="h-4 w-4" /> رفض</button>
                </div>
              </div>
            ))}
          </div>
            ...
            <div className="rounded-xl border border-white/10 bg-ink-900 p-3">
              {isImageFile(preview.file_type) ? (
                ...
                <div className="py-12 text-center">
                  ...
                  <a href={preview.file_url} target="_blank" rel="noreferrer" className="btn-ghost mt-4"><Icon name="Download" className="h-4 w-4" /> فتح الملف</a>
                </div>
              )}
            </div>
            <div>
              <h3 className="font-bold text-slate-100">{preview.title}</h3>
              <p className="text-sm text-slate-400">{preview.uploader?.full_name} · {preview.subject?.name}</p>
            </div>
            <div className="flex justify-end gap-2">
              <button onClick={() => reject(preview.id, preview.storage_path)} className="btn-danger"><Icon name="Trash2" className="h-4 w-4" /> رفض وحذف</button>
              <button onClick={() => approve(preview.id)} className="btn-primary"><Icon name="Check" className="h-4 w-4" /> موافقة ونشر</button>
            </div>
          </div>
        )}
      </Modal>
...
function RoleBadge({ role }: { role: string }) {
  ...
  return (<span className={`badge border ${r.cls}`}><Icon name={r.icon} className="h-3 w-3" /> {r.label}</span>);
}

function isImageFile(type?: string | null) { return !!type && ['png','jpg','jpeg','gif','webp','svg'].includes(type.toLowerCase()); }
function isPdfFile(type?: string | null) { return !!type && type.toLowerCase() === 'pdf'; }
