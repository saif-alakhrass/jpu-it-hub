...
export function HomePage() {
  ...
  async function load() {
    ...
    const { data, error } = await supabase
      .from('subjects')
      .select('id, name, description, major, created_by, created_at')
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      <section className="relative overflow-hidden card mb-10 p-8 md:p-12">
        ...
        <div className="relative">
          <span className="badge bg-brand-500/15 text-brand-300 border border-brand-500/30 mb-4">
            <Icon name="Sparkles" className="h-3.5 w-3.5" /> منصة الطلاب الذكية
          </span>
      ...
      <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div className="relative flex-1 max-w-md">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ابحث عن مادة..." className="input pr-11" />
        </div>
        <div className="flex items-center gap-2 overflow-x-auto pb-1">
          <Icon name="Filter" className="h-4 w-4 shrink-0 text-slate-500" />
          {['الكل', ...MAJORS].map((m) => (
            <button key={m} onClick={() => setMajor(m)} className={`whitespace-nowrap rounded-full px-3.5 py-1.5 text-xs font-bold transition ${major === m ? 'bg-brand-500 text-ink-950' : 'bg-ink-800 text-slate-300 hover:bg-ink-700 border border-white/5'}`}>
              {m}
            </button>
          ))}
        </div>
        <button onClick={() => session ? setCreateOpen(true) : navigate('/auth')} className="btn-primary shrink-0">
          <Icon name="Plus" className="h-4 w-4" /> مادة جديدة
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((s) => (
            <button key={s.id} onClick={() => navigate(`/subject/${s.id}`)} className="card group p-5 text-right transition-all duration-200 hover:-translate-y-1 hover:border-brand-500/40 hover:shadow-glow">
              ...
              <h3 className="mb-1 text-lg font-bold text-slate-100 group-hover:text-brand-300 transition">{s.name}</h3>
              <p className="text-sm text-slate-400 line-clamp-2 min-h-[2.5rem]">{s.description ?? 'لا يوجد وصف'}</p>
              <div className="mt-3 flex items-center gap-1 text-xs text-brand-400 font-bold opacity-0 transition group-hover:opacity-100">
                عرض الموارد <Icon name="ChevronLeft" className="h-4 w-4" />
              </div>
            </button>
          ))}
        </div>
      ...
      <Modal open={createOpen} onClose={() => setCreateOpen(false)} title="إنشاء مادة جديدة">
        <form onSubmit={handleCreate} className="space-y-4">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="مثال: برمجة 1" className="input" />
          </div>
          <div>
          ...
          <div>
            ...
            <textarea value={desc} onChange={(e) => setDesc(e.target.value)} rows={3} placeholder="نبذة قصيرة عن المادة..." className="input resize-none" />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => setCreateOpen(false)} className="btn-ghost">إلغاء</button>
            <button type="submit" disabled={submitting} className="btn-primary">
              {submitting ? <Icon name="Loader2" className="h-4 w-4 animate-spin" /> : <Icon name="Plus" className="h-4 w-4" />} إنشاء
            </button>
          </div>
        </form>
      </Modal>
