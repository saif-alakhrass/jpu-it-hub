...
export function SubjectPage({ subjectId }: { subjectId: string }) {
  ...
  async function loadSubject() {
    const { data } = await supabase.from('subjects').select('id, name, description, major, created_by, created_at').eq('id', subjectId).maybeSingle();
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    if (!title || !file) { setToast({ message: 'يرجى إدخال العنوان واختيار الملف', type: 'error' }); return; }
    ...
    if (upErr) { setToast({ message: 'فشل رفع الملف: ' + upErr.message, type: 'error' }); return; }
    const { data: pub } = supabase.storage.from('files').getPublicUrl(path);
    const { error: insErr } = await supabase.from('files').insert({ subject_id: subjectId, tab: activeTab, title, storage_path: path, file_url: pub.publicUrl, file_type: ext });
    if (insErr) { setToast({ message: 'فشل حفظ الملف: ' + insErr.message, type: 'error' }); return; }
    setToast({ message: canPublishDirectly ? 'تم نشر الملف مباشرة' : 'تم رفع الملف وهو قيد المراجعة', type: 'success' });
    setUploadOpen(false); form.reset(); await loadFiles();
  }

  if (loading) return (<div className="mx-auto max-w-5xl px-4 py-20 text-center"><Icon name="Loader2" className="mx-auto h-8 w-8 animate-spin text-brand-400" /></div>);
  if (!subject) return (
    <div className="mx-auto max-w-5xl px-4 py-20 text-center">
      ...
      <button onClick={() => navigate('/')} className="btn-ghost mt-4"><Icon name="Home" className="h-4 w-4" /> العودة للرئيسية</button>
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <button onClick={() => navigate('/')} className="mb-5 flex items-center gap-1 text-sm text-slate-400 hover:text-brand-300 transition">
        <Icon name="ChevronLeft" className="h-4 w-4" /> العودة للمواد
      </button>
      <header className="card mb-6 p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <button onClick={() => session ? setUploadOpen(true) : navigate('/auth')} className="btn-primary shrink-0">
            <Icon name="Upload" className="h-4 w-4" /> رفع ملف
          </button>
        </div>
      </header>
      <div className="mb-6 flex gap-2 overflow-x-auto border-b border-white/5 pb-px">
        {TABS.map((t) => (
          <button key={t.key} onClick={() => setActiveTab(t.key)} className={`flex items-center gap-2 whitespace-nowrap rounded-t-xl border-b-2 px-4 py-3 text-sm font-bold transition ${activeTab === t.key ? 'border-brand-500 text-brand-300' : 'border-transparent text-slate-400 hover:text-slate-200'}`}>
      ...
      {tabFiles.length === 0 ? (
        <div className="card p-12 text-center">
          ...
          {session && (<button onClick={() => setUploadOpen(true)} className="btn-ghost mt-4"><Icon name="Upload" className="h-4 w-4" /> ارفع أول ملف</button>)}
        </div>
      ) : (
          ...
          {tabFiles.map((f) => {
            ...
            return (
              <div key={f.id} className={`card flex items-center gap-4 p-4 transition hover:border-white/10 ${pending && !isOwn ? 'opacity-50' : ''}`}>
                <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-ink-700 text-brand-400"><Icon name="File" className="h-5 w-5" /></span>
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <h3 className="truncate font-bold text-slate-100">{f.title}</h3>
                    {pending && (<span className="badge bg-accent-500/15 text-accent-400 border border-accent-500/30"><Icon name="Clock" className="h-3 w-3" /> قيد المراجعة</span>)}
                  </div>
                  <div className="mt-0.5 flex items-center gap-3 text-xs text-slate-500">
                    <span>{f.uploader?.full_name ?? 'مستخدم'}</span><span>·</span>
                ...
                {!pending || isOwn ? (
                  <a href={f.file_url} target="_blank" rel="noreferrer" className="btn-ghost shrink-0" title="معاينة / تنزيل">
                    <Icon name="Eye" className="h-4 w-4" /><span className="hidden sm:inline">عرض</span>
                  </a>
                ) : (
                  <span className="badge bg-ink-700 text-slate-500 border border-white/5 shrink-0"><Icon name="Lock" className="h-3 w-3" /> مخفي</span>
                )}
              </div>
            );
          })}
        ...
        <form onSubmit={handleUpload} className="space-y-4">
          {!canPublishDirectly && (
            <div className="flex items-start gap-2 rounded-xl border border-accent-500/30 bg-accent-500/10 p-3 text-sm text-accent-400">
              <Icon name="AlertCircle" className="h-5 w-5 shrink-0" /><span>ستحتاج ملفاتك إلى موافقة المدير قبل نشرها للجميع.</span>
            </div>
          )}
          <div>
          ...
          <div>
            ...
            <input name="file" type="file" required className="block w-full text-sm text-slate-300 file:ms-0 file:me-3 file:rounded-lg file:border-0 file:bg-brand-500 file:px-4 file:py-2 file:text-ink-950 file:font-bold hover:file:bg-brand-400" />
          </div>
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => setUploadOpen(false)} className="btn-ghost">إلغاء</button>
            <button type="submit" className="btn-primary"><Icon name="Upload" className="h-4 w-4" /> رفع</button>
          </div>
        </form>
      </Modal>
