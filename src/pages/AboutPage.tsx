...
function PillarCard({
  ...
  const iconBg =
    accent === 'accent'
      ? 'bg-accent-500/15 text-accent-400 group-hover:bg-accent-500 group-hover:text-ink-950'
      : 'bg-brand-500/15 text-brand-400 group-hover:bg-brand-500 group-hover:text-ink-950';
  return (
    ...
    <Reveal delay={delay} y={32} className="h-full">
      <div className="card group relative flex h-full flex-col overflow-hidden p-7 text-center transition-all duration-300 hover:-translate-y-1 hover:border-brand-500/40 md:p-9">
        ...
        <div className="relative flex flex-1 flex-col items-center">
              ...
              <div className="h-full w-full rounded-full bg-ink-900 p-1">
                <div className="grid h-full w-full place-items-center overflow-hidden rounded-full bg-ink-800">
                  {hasPhoto ? (
                    ...
                    <Icon name="User" className="h-14 w-14 text-slate-600 md:h-16 md:w-16" />
                  )}
                </div>
              </div>
            </div>
            {verified && (
              <span className="absolute bottom-1.5 left-1.5 grid h-7 w-7 place-items-center rounded-full border-2 border-ink-900 bg-brand-500 text-ink-950">
          ...
          {member.bio && (
            <p className="mt-3 max-w-xs text-xs leading-relaxed text-slate-400 md:text-sm">{member.bio}</p>
          )}
            ...
            <div className="mt-auto flex flex-wrap items-center justify-center gap-2 pt-6">
              {links.map((l) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="btn-ghost group/btn !px-3 !py-2 text-xs"
                >
                  <Icon name={l.icon} className="h-3.5 w-3.5 transition group-hover/btn:text-brand-400" />
                  <span>{l.label}</span>
                </a>
              ))}
            </div>
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <section className="relative overflow-hidden card mb-14 p-8 text-center md:p-16">
        ...
        <div className="relative">
          <Reveal y={20}>
            <span className="badge mb-5 border border-brand-500/30 bg-brand-500/15 text-brand-300">
              <Icon name="Sparkles" className="h-3.5 w-3.5" />
              منصة الطلاب الذكية
            </span>
          </Reveal>
          <Reveal delay={80} y={24}>
            <h1 className="text-3xl font-extrabold leading-tight text-slate-100 md:text-5xl">
              عن منصة <span className="text-brand-400">JPU-IT Hub</span>
            </h1>
          </Reveal>
          <Reveal delay={160} y={20}>
            <p className="mx-auto mt-4 max-w-2xl text-slate-400 md:text-lg">
              نُمكّن طلاب كلية تكنولوجيا المعلومات في جامعة جرش من خلال تجميع الموارد الأكاديمية
              في مكان واحد — ملخصات، امتحانات سابقة، سلايدات، وكتب — يرفعها الطلاب أنفسهم
              ويراجعها المشرفون لضمان جودة محتوى يعود بالنفع على الجميع.
            </p>
          </Reveal>
      ...
      <section id="story" className="mb-16 scroll-mt-24">
        <Reveal className="mb-10 text-center">
          <span className="badge mb-3 border border-white/5 bg-ink-800 text-brand-300">
            <Icon name="Quote" className="h-3.5 w-3.5" />
            قصتنا
          </span>
          <h2 className="text-2xl font-extrabold text-slate-100 md:text-4xl">من أين بدأنا؟</h2>
        </Reveal>
        <Reveal delay={80} y={28}>
          <div className="card mx-auto max-w-3xl p-7 text-center md:p-10">
            <p className="text-base leading-loose text-slate-300 md:text-lg">
              انطلقت فكرة المنصة من حاجة بسيطة يعيشها كل طالب: البحث عن ملخص قبل الامتحان،
              أو سلايدات أستاذ معيّن، أو نموذج امتحان سابق — في ظلّ تشتّت المصادر بين مجموعات
              الواتساب ومحركات البحث. فكان <span className="font-bold text-brand-400">JPU-IT Hub</span>:
              مكانٌ واحد منظّم، يبنيه الطلاب بأنفسهم، وتُراجَع فيه الملفات قبل نشرها ليبقى المحتوى
              موثوقًا ونظيفًا. اليوم نغطّي تخصصي علم الحاسوب والأمن السيبراني، ونطمح لأكثر.
            </p>
          </div>
        </Reveal>
      </section>
      ...
      <section className="mb-16">
        <Reveal className="mb-10 text-center">
          <span className="badge mb-3 border border-white/5 bg-ink-800 text-accent-400">
            <Icon name="Target" className="h-3.5 w-3.5" />
            ركائزنا
          </span>
          <h2 className="text-2xl font-extrabold text-slate-100 md:text-4xl">على ماذا نبني؟</h2>
        </Reveal>
        <div className="grid gap-5 md:grid-cols-3">
          <PillarCard icon="Telescope" title="رؤيتنا" accent="brand" delay={0}>
            أن نكون المرجع الأوّل لطلاب تكنولوجيا المعلومات في جامعة جرش — منصّة يثق بها
            الطالب ويجد فيها كل ما يحتاجه بضغطة زر، فيوفّر وقته ويركّز على تعلّمه.
          </PillarCard>
          <PillarCard icon="Rocket" title="رسالتنا" accent="accent" delay={120}>
            تسهيل الوصول إلى الموارد الأكاديمية وتنظيمها، وتمكين الطلاب من المشاركة في بناء
            قاعدة معرفة جماعية مراجَعة، تخدم الجيل الحالي والأجيال القادمة من الطلاب.
          </PillarCard>
          <PillarCard icon="Gift" title="ما نقدّمه للطلاب" accent="brand" delay={240}>
            أربعة أقسام لكل مادة: تلاخيص وشروحات، امتحانات وسنوات سابقة، صور ومسودات،
            وسلايدات وكتب — مع نظام رفع بسيط ومراجعة تضمن جودة المحتوى المنشور.
          </PillarCard>
        </div>
      </section>
      ...
      <section className="mb-16">
        <Reveal className="mb-8 text-center">
          <h2 className="text-2xl font-extrabold text-slate-100 md:text-3xl">مزايا تجدها فقط هنا</h2>
        </Reveal>
      ...
      {/* Founder & Assistant — side-by-side */}
      <section className="mb-14">
        <Reveal className="mb-8 text-center">
          <span className="badge mb-3 border border-brand-500/30 bg-brand-500/15 text-brand-300">
            <Icon name="Code" className="h-3.5 w-3.5" />
            فريق العمل
          </span>
          <h2 className="text-2xl font-extrabold text-slate-100 md:text-4xl">مطوّرو المنصة</h2>
          <p className="mx-auto mt-3 max-w-xl text-sm text-slate-400">
            الفريق الذي يقف خلف بناء المنصة وتطويرها والإشراف على محتواها.
          </p>
        </Reveal>
        ...
        {loading ? (
          <div className="card mx-auto max-w-2xl p-12 text-center">
            ...
            <p className="mt-3 text-sm text-slate-400">جارٍ تحميل بيانات الفريق…</p>
          </div>
        ) : error ? (
          <div className="card mx-auto max-w-2xl p-10 text-center">
            <Icon name="AlertCircle" className="mx-auto h-8 w-8 text-danger-400" />
      ...
      {staff.length > 0 && (
        <section className="mb-16">
          <Reveal className="mb-8 text-center">
            ...
            <h2 className="text-2xl font-extrabold text-slate-100 md:text-3xl">فريق المنصة</h2>
          </Reveal>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {staff.map((m, i) => (
              <PersonCard key={m.id} member={m} delay={i * 80} />
            ))}
          </div>
        </section>
      )}
        ...
        <div className="card relative overflow-hidden p-8 text-center md:p-12">
          ...
          <div className="relative">
            ...
            <p className="mx-auto mt-2 max-w-lg text-slate-400">
              تصفّح المواد، ارفع ملخصك، وكن جزءًا من مجتمع يبني قاعدة المعرفة لطلاب الجامعة.
            </p>
