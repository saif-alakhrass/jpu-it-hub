...
2. Copy `.env.example` to `.env` and fill in:
   ...
   - `VITE_R2_WORKER_URL` — your deployed Cloudflare Worker URL (e.g. `https://jpu-it-hub-storage.your-subdomain.workers.dev`)
3. `npm install`
4. `npm run dev`
...
- The `role` column on `profiles` is protected by a database trigger: only admins can change user roles
...
```bash
# This script reads all files with storage_provider = 'supabase',
# downloads them from Supabase Storage, uploads to R2, and updates
# the database record. It does NOT delete the Supabase Storage objects.
#
# Run in a Node.js environment with access to both Supabase and R2.
...
1. **Frontend**: Remove `VITE_R2_WORKER_URL` from Vercel env vars. The app will automatically fall back to Supabase Storage for all files (legacy path).
2. **Database**: The `storage_provider`, `object_key`, `file_hash`, and `mime_type` columns are nullable and default to `'supabase'`. No data loss occurs — old files continue working.
